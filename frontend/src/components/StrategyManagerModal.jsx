...
export default function StrategyManagerModal({ isOpen, onClose, strategies = [], onCreateStrategy, onDeleteStrategy }) {
  ...
  const handleCreate = (e) => {
    e.preventDefault();
    if (!name.trim()) return;
    onCreateStrategy({ name: name.trim(), description: description.trim() });
    setName('');
    setDescription('');
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="card-dark w-full max-w-xl border border-slate-700/80 my-8 shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleCreate} className="bg-[#151921] p-4 rounded-xl border border-slate-800 space-y-3">
            <h4 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Add New Strategy</h4>
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Existing Strategies</h4>
